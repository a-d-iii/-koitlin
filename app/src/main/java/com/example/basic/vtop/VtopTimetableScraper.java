...
public class VtopTimetableScraper {
    ...
    /**
     * Perform the full login sequence using the built-in captcha solver.
     */
    public void login() throws IOException {
        ...
        if (cap.imageBase64 != null) {
            try {
                captchaValue = CaptchaSolver.solve(cap.imageBase64);
            } catch (Exception e) {
                captchaValue = null;
            }
        }
        if (captchaValue == null) {
            throw new IOException("Failed to solve captcha");
        }
